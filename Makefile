...
ifdef DETTRACE_NO_CPUID_INTERCEPTION
	docker run --rm --privileged --cap-add=SYS_ADMIN --env DETTRACE_NO_CPUID_INTERCEPTION=1  ${DOCKER_NAME}:${DOCKER_TAG} bin/dettrace --debug 4 test/samplePrograms/twoPthreadsNoJoin.bin
else
	docker run --rm --privileged --cap-add=SYS_ADMIN ${DOCKER_NAME}:${DOCKER_TAG} bin/dettrace --debug 4 test/samplePrograms/twoPthreadsNoJoin.bin
endif
