...
static void handler(int sig, siginfo_t *si, void *ctxt) {
  printf("Received signal %d\n siginfo_t fields: signo:%d errno:%d code:%d pid:%d uid:%d status:%d utime:%ld stime:%ld value:%p int:%d ptr:%p overrun:%d timerid:%d addr:%p band:%ld fd:%d addr_lsb:%d lower:%p upper:%p pkey:%d call_addr:%p syscall:%d uarch:%u\n",
         sig, 
         ...
         si->si_addr, si->si_band, si->si_fd, si->si_addr_lsb, si->si_lower, si->si_upper, si->si_pkey,
