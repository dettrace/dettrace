...
int main() {
  ...
  int signalToHandle;
#if WHICH_ITIMER == ITIMER_REAL
  signalToHandle = SIGALRM;
#elif WHICH_ITIMER == ITIMER_VIRTUAL
  signalToHandle = SIGVTALRM;
#elif WHICH_ITIMER == ITIMER_PROF
  signalToHandle = SIGPROF;
#else
  #error Unsupported WHICH_ITIMER value
#endif
  
  int rv = sigaction(signalToHandle, &sa, NULL);
