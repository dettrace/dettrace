...
/**
 * This class wraps raw memory which itself represents either struct linux_dirent or struct linux_dirent64.
 * That is, directory entries for the system calls getdents and getdents64.
 *
...
public:
  ...
  /**
   * Return an array of size < bytesNeeded, in order to fill it with as many entries as possible.
...
private:
  /**
   * This vector represents contigious linux_dirent entries as a raw array of bytes.
   */
  ...
  /**
   * Turn our entries into a vector of (name, address, byteSize) for easy sorting.
   * Each address is it's location on our array this way, we can "sort" the variable
